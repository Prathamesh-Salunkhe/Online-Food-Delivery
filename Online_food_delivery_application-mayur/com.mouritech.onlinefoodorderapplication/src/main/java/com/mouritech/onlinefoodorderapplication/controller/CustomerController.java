...
public class CustomerController {
	...
	@PostMapping("/insertcartsBycustomeremailandpassword")
	public ResponseEntity<?>insertCarts(@RequestBody CustomerCartsDto customerCartsDto){
		
		if(customerCartsDto==null) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("please Enter Email and password, items");
		}
		else {
			
			ResponseEntity<?> cartsInsert = customerService.insertCarts(customerCartsDto);
			return cartsInsert;
